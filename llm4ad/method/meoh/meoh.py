...
class MEoH:
    ...
    def _continue_sample(self):
        """Check if it meets the max_sample_nums and max_generations restrictions.
        Returns True if sampling should continue, False otherwise.
        """
        # Nếu cả hai giới hạn đều không được đặt, luôn tiếp tục lấy mẫu
        if self._max_generations is None and self._max_sample_nums is None:
            return True

        # Nếu chỉ đặt giới hạn số lượng mẫu, tiếp tục nếu chưa đạt tới giới hạn
        ...
        # Nếu chỉ đặt giới hạn số thế hệ, tiếp tục nếu chưa đạt tới giới hạn
        ...
        # Nếu cả hai giới hạn đều được đặt, chỉ tiếp tục khi cả hai điều kiện đều chưa đạt tới giới hạn
