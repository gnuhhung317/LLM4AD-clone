# Module Name: evaluation4Violations
# Last Revision: 2025/5/30
# Description: Three-objective evaluation for the Vehicle Routing Problem with Time Windows (VRPTW).
#       This module evaluates VRPTW solutions based on three objectives:
#       1. Total travel distance (to be minimized)
#       2. Total time window violations/waiting time (to be minimized)
#       3. Algorithm execution time (to be minimized)
...
class Evaluation4Violations(Evaluation):
    def __init__(self,
        ...
        get_data_generator = GetData(self.n_instance, self.problem_size + 1)
        self._datasets = get_data_generator.generate_instances()
    ...
    def solve_single_instance(self, heuristic, instance_data):
        ...
        algorithm_end_time = time.time()
        total_execution_time = algorithm_end_time - algorithm_start_time
        ...
        return total_distance, total_violations, total_execution_time
    ...
    def evaluate(self, heuristic):
        ...
        for instance_data in self._datasets:
            ...
            total_distance, total_violations, total_execution_time = result
            distances.append(total_distance)
            violations.append(total_violations)
            execution_times.append(total_execution_time)

        # Return all three objectives as negative values (for maximization in MEoH)
        avg_distance = np.average(distances)
        avg_violations = np.average(violations)
        avg_execution_time = np.average(execution_times)
        
        # Return as numpy array with three objectives
...
if __name__ == '__main__':
    ...
    eval_instance = Evaluation4Violations()
    result = eval_instance.evaluate_program('', select_next_node)
    print(f"Three-objective result: {result}")
    if result is not None:
        print(f"Objective 1 (negative distance): {result[0]}")
        print(f"Objective 2 (negative violations): {result[1]}")
        print(f"Objective 3 (negative execution time): {result[2]}")
