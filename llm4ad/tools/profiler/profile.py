    ...
    def _record_and_print_verbose(self, function, *, resume_mode=False):
        ...
        if self._num_objs < 2:
            ...
            if score is not None:
                # Handle case where score might be a scalar even in multi-objective mode
                if not hasattr(score, '__len__'):
                    # Convert scalar to array by replicating the score
                    print(f"Warning: Expected {self._num_objs} objectives but got scalar score: {score}. Converting to multi-objective format.")
                    score = [score] * self._num_objs
                elif len(score) != self._num_objs:
                    print(f"Warning: Expected {self._num_objs} objectives but got {len(score)} objectives: {score}")
                    return
